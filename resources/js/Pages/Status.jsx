...
export default function Status({ auth, data }) {
    function interpolateColor(progress) {
        ...
        const pureGreen = [0, 255, 0];
        
        // Interpolate between pure red and pure green
        const lerpedColor = pureRed.map((channel, index) =>
          Math.round(channel * (1 - progress) + pureGreen[index] * progress)
        );
        
        return `rgb(${lerpedColor[0]}, ${lerpedColor[1]}, ${lerpedColor[2]})`;
    }
                                    ...
                                    <div className='scale-100 p-2 gap-1 bg-white dark:bg-gray-800/50 dark:bg-gradient-to-bl from-gray-700/50 via-transparent dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-lg shadow-2xl shadow-gray-500/20 dark:shadow-none flex transition-all duration-250 focus:outline focus:outline-2 focus:outline-red-500'>
                                        {
                                            value.map((x, i) => 
                                                ...
                                                data-tooltip-html={"Event Recorded: " + x.time + "<br />Health: " + x.health + "%<br />" + "Number of Events: " + x.poll}
                                                data-tooltip-place="top"
                                                >
                                                    <div className='flex-auto py-5 px-1' style={{backgroundColor: interpolateColor(x.health/100)}}></div>
                                                </a>
                                            )
                                        }
                                    </div>
